<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold text-gray-800">{{ $t('tripList.myTrips') }}</h2>
    </div>
    <div
      v-if="trips.length === 0"
      class="text-center text-gray-500 py-10 text-base"
    >
      <p class="mb-4">{{ $t('tripList.noTrips') }}</p>
    </div>
    <ul class="space-y-4">
      <li
        v-for="trip in trips"
      ...
      >
        ...
        >
          {{ $t('tripList.noCoverImage') }}
        </div>
        <div class="p-4">
          <h3 class="font-bold text-lg text-gray-800 mb-1">{{ trip.title }}</h3>
          <p class="text-gray-600 text-sm">
            {{ trip.startDate }} ~ {{ trip.endDate }} ({{$t('tripList.total') }}
            {{ trip.days.length }} {{ $t('tripList.days') }})
          </p>
        </div>
      </li>
    </ul>
...
defineEmits(["view-trip", "create-new-trip"]);
</script>
