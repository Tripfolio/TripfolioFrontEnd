<template>
  ...
  >
    <div
      class="backdrop-blur-lg bg-[#878787]/30 w-full max-w-md rounded-2xl p-6 shadow-xl relative"
    >
      <button
        @click="$emit('close')"
      ...
      <div
        v-if="result === 'success'"
        class="text-center text-white p-6 rounded"
      >
        <h2 class="text-2xl font-bold mb-2">{{ $t('paymentModal.success') }}</h2>
        <p class="text-white mb-4">{{ $t('paymentModal.successMessage') }}</p>
        <button
          @click="$emit('close')"
          class="bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded shadow"
        >
      ...
      >
        ...
        <p class="text-white mb-4">
          {{ $t('paymentModal.failMessage') }}
        </p>
        <button
          @click="$emit('close')"
          class="bg-red-600 hover:bg-red-500 text-white px-6 py-2 rounded shadow"
        >
          關閉
        </button>
      </div>
      ...
      <div v-else>
        ...
        <h1 class="text-2xl font-bold mb-4 text-center text-white">
          {{ $t('paymentModal.upgradeToPremium') }}
        </h1>
        <p class="mb-6 text-center text-white">
          {{ $t('paymentModal.payAmount') }}
          <span class="font-semibold text-xl">NT$ {{ paymentAmount }}</span>
          {{ $t('paymentModal.moreTrips') }}
        </p>

        <div
          v-if="isPremium"
          class="text-center bg-green-50 border border-green-300 text-green-700 p-4 rounded mb-4"
        >
        ...
        <div v-else>
          ...
          <div class="border-t border-gray-300 pt-4 mt-4 text-center">
            ...
            <div v-if="showCreditCardDetails" class="mt-4">
              ...
              <div
                id="dropin-container"
                class="border rounded p-2 bg-gray-50 mb-4"
              ></div>
              <div class="space-y-2">
                <button
                  v-if="!dropinisReady"
...
const checkPremium = async () => {
  ...
  try {
    const res = await axios.get(
      `${import.meta.env.VITE_API_URL}/api/linepay/check-status`,
      {
