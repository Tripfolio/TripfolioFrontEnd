...
const updateTitle = async (newTitle) => {
    try {
        await axios.put(`${import.meta.env.VITE_API_URL}/api/travelSchedule/${props.tripId}`, 
        { title: newTitle },
        { headers: { Authorization: `Bearer ${token}` } }
    )
...
const updateDates = async ({ startDate, endDate }) => {
    try {
        await axios.patch(`${import.meta.env.VITE_API_URL}/api/travelSchedule/${props.tripId}`,
            { startDate, endDate },
            { headers: { Authorization: `Bearer ${token}`} }
        )
