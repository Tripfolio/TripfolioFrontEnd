<template>
  <div class="max-w-md mx-auto mt-12 p-6 bg-white rounded-xl shadow">
    <div v-if="loading" class="text-center text-gray-500">{{ t('getInvite.loading') }}</div>
    ...
    <div v-else>
      ...
      <p class="text-gray-600 text-center mb-4">
        {{ $t('getInvite.host') }}：{{ trip.ownerName }}｜{{ $t('getInvite.youWillHave') }}
        <span class="font-semibold">
          {{ trip.permission === "editor" ? $t('getInvite.editPermission') : $t('getInvite.viewPermission') }}
        </span>
      </p>

      <div v-if="joined" class="text-green-600 text-center font-medium">
        {{ t('getInvite.alreadyJoined') }}
      </div>
...
const fetchInviteInfo = async () => {
  try {
    ...
    if (err.response?.status === 403 || err.response?.status === 404) {
      error.value = $t('getInvite.linkExpiredOrInvalid');
    } else {
      error.value = $t('getInvite.failedToLoadTripInfo');
    }
  } finally {
    loading.value = false;
  }
